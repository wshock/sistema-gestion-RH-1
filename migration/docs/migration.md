...
**pgloader no traslada la propiedad de autogeneración.** Copia los valores de
las columnas IDENTITY de SQL Server, pero las deja como enteros normales: sin
secuencia y sin identity. Verificado sobre la base migrada, `pg_get_serial_sequence`
devolvía `NULL` para las cuatro tablas. El síntoma es que cualquier `INSERT`
que no especifique la primary key falla, lo que impide crear registros desde
la aplicación.

`reset_sequences.sql` detecta esas 4 tablas (PK de una sola columna entera
que no sea a la vez foreign key) y para cada una:

1. Si le falta la autogeneración, se la añade con
   `ADD GENERATED BY DEFAULT AS IDENTITY (START WITH MAX(id) + 1)`. Se usa
   `BY DEFAULT` y no `ALWAYS` para no romper cargas que sí traen id explícito.
2. Si ya tenía secuencia, la reajusta a `MAX(id)` con `setval`.
3. Inserta una fila de prueba (usando `DEFAULT` para el id y valores dummy
   para el resto de columnas `NOT NULL` sin default) para confirmar que la
   secuencia ya no colisiona con datos existentes.
4. Elimina inmediatamente la fila de prueba insertada.

Tras correrlo hay que reflejarlo en Prisma: esas cuatro PK llevan
`@default(autoincrement())` en `schema.prisma`.
